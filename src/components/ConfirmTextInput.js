...
const ConfirmTextInput = (props) => {
  ...
  return (
    ...
    <div>
        <div 
        className={`flex justify-center items-center p-1 mt-4 rounded-md border-2 ${error ? 'border-red-500' : 'border-gray-100'}`}>
          <input
            type="text"
            value={review}
          ...
          <button className='border-none rounded-md px-2 py-2 cursor-pointer font-normal transition-all ease-in duration-100 bg-slate-300 disabled:cursor-default disabled:bg-slate-100 disabled:text-slate-500 hover:enabled:bg-dark-green hover:enabled:text-white' onClick={handleConfirm} disabled={disabled}>Conferma</button>
        </div>
    </div>
